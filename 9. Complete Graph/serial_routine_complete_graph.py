...
def heatmap2d(arr: np.ndarray, time, beta, step_function_text):
...
def grid_eval(time_array, beta_array):
    ...
    for j in range(len(beta_array)):
        ...
        print("iter: ", j+1,"/",len(beta_array))
    return probability

def serial_routine(lb_time, ub_time, lb_beta, ub_beta):

    tic = time.perf_counter()

    time_sampling_points = 40
    beta_sampling_points = 30
    beta_array = np.linspace(lb_beta, ub_beta, beta_sampling_points)
    time_array = np.linspace(lb_time, ub_time, time_sampling_points)


    probability = grid_eval(time_array, beta_array)
    ...
    file_probability = str(dimension) + '_s_cg_probability_' + step_function_text[step_function-1]
    file_time = str(dimension) + '_s_cg_time_array_.npy'
    file_beta = str(dimension) + '_s_cg_beta_array_.npy'

    #export heatmap plot
    heatmap2d(probability, time_array, beta_array, step_function_text)

    np.save(file_probability, probability)
    np.save(file_time, time_array)
    np.save(file_beta, beta_array)
...
if __name__ == '__main__':
    ...
    dimension = 15
    serial_routine(0, dimension*2, 0, 10)
