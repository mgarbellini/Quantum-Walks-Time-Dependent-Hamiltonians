...
def routine():
    ...
    beta = np.linspace(dimension - 0.5*dimension,dimension + 0.5*dimension,20)
    oracle_site_state = np.empty([dimension, 1])
    oracle_site_state.fill(0)
    oracle_site_state[int((dimension - 1) / 2)][0] = 1
    ...
    for i in range(len(beta)):
        probability =  -evaluate_probability([beta[i],float(np.pi/(2*np.sqrt(dimension)))], oracle_site_state)
        if(probability > max):
            max = probability

    print(dimension, probability[0], 1/probability[0])
...
if __name__ == '__main__':
    ...
    for dim in dimensions:
        dimension = dim
        max_time = np.pi/(2*np.sqrt(dimension))

        type_of_qw = 'independent'
        step_function = step_functions[0]
        parallel_routine(0, max_time, 0, 2*dimension )
