...
def generate_hamiltonian(dimension, beta, time, T):
    ...
    if(time == 0 or T == 0):
        ...
        if(step_function == 1):
            ...
            g_T = (nn * nn + nn * np.tan(2 * nn * time *
                                         (1 / dimension) - np.arctan(nn)) * (1 / (2 * nn * nn)))
        elif(step_function == 5):
...
def heatmap2d(probability, time, beta, step_function_text):
    ...
    title = 'Dynamic Prob Complete Graph N=' + \
        str(dimension) + ' (' + step_function_text[step_function - 1] + ')'
    ...
    pdf = '.pdf'
    file_name = str(dimension) + '_cg_heatmap_' + \
        step_function_text[step_function - 1] + pdf
...
def parallel_routine(lb_time, ub_time, lb_beta, ub_beta):
    ...
    step_function_text = ["lin", "sqrt", "cbrt", "cerf_tan", "cerf"]
    npy = ".npy"

    file_probability = 'FIG7_'+ str(dimension) + '_circ_prob_' + step_function_text[step_function - 1]
    file_time = 'FIG7_' +str(dimension) + '_circ_time.npy'
    file_beta = 'FIG7_'+ str(dimension) + '_circ_beta.npy'
